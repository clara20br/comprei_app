...
// Registrar adaptadores (sem usar .g.dart)
...
Hive.registerAdapter(ConfiguracaoAdapter());
  ...
  static Future<void> salvarLista(ListaCompras lista) async => await _listasBox.put(lista.id, lista);
  static Future<void> deletarLista(String id) async => await _listasBox.delete(id);
  ...
  static Future<void> salvarCategoria(Categoria categoria) async => await _categoriasBox.put(categoria.id, categoria);
  static Future<void> deletarCategoria(String id) async => await _categoriasBox.delete(id);
  ...
  static Future<void> salvarItem(Item item) async => await _itensBox.put(item.id, item);
  static Future<void> deletarItem(String id) async => await _itensBox.delete(id);
  
  // Histórico
  static List<HistoricoPreco> get historico => _historicoBox.values.toList();
  static Future<void> salvarHistorico(HistoricoPreco historico) async => await _historicoBox.put(historico.id, historico);
  
  // Configuração
  static Configuracao get configuracao => _configBox.get('config') ?? Configuracao();
  static Future<void> salvarConfiguracao(Configuracao config) async => await _configBox.put('config', config);
}